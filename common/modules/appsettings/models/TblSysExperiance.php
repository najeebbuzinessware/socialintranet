 ...
 */
class TblSysExperiance extends CActiveRecord
{
	/**
	...
	public function rules()
	{
		...
		return array(
			array('MID, Experiance', 'required'),
			array('MID, CreatedBy, ModifiedBy', 'numerical', 'integerOnly'=>true),
			array('Experiance', 'length', 'max'=>255),
			array('IsActive, IsDelete', 'length', 'max'=>4),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('MaxExperianceId, MID, Experiance, CreatedBy, CreatedOn, ModifiedBy, ModifiedOn, IsActive, IsDelete', 'safe', 'on'=>'search'),
		);
	}
