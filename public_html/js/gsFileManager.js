...
gs_filemanager_languages['en'][45] = 'Share';
gs_filemanager_languages['en'][46] = 'Share File';
gs_filemanager_languages['en'][47] = 'Please Select User';
...
if (jQuery) (function(jQuery){
	...
	jQuery.extend(jQuery.fn, {
		gsFileManager: function(o) {
			...
			var menuHtml = '<table class=\'gsHeadTable\' cellpadding=0 cellspacing=0><tr><td><span> ' + gs_getTranslation(o.language, 1)+ ': </span><span id=\'curDir\'></span></td></tr></table>';
			...
			contexMenus += '<li class="rename"><a href="#Share" rel="46">' + gs_getTranslation(o.language, 46)+ '</a></li>';
			...
			hiddenElements += '</div>';
			hiddenElements += '<div id="sharefiles" style=\'display: none; position: relative;\'>';
			hiddenElements += '<form action="/fileManager/shareFile" id="gsShareForm" enctype="multipart/form-data"><input type="hidden" name="opt" value="46"><input type="hidden" name="dir" value=""><input type="hidden" name="filename" value="">';
			hiddenElements +=  '<div class="fileinputs" id="ShareFormFields"></div><div id="ShareError" class="error"></div></form>';
			hiddenElements += '</div>';
			...
			function showFiles (gsfiless) {
				...
				if (gsfiless.length > 0) {
					for (var num in gsfiless) {
						...
						fileshtml += "<tr><td><div class='file gsItem directory_info ext_" + curItem.getExt() + "' rel=\'" + curItem.id + "\'>" + curItem.name + "</div><div id=\'" + curItem.id + "\'></div></td><td><span class=\'file_ext_name\'>" + curItem.getExt() + "</span> file</td><td>" + curItem.getSize() + "</td><td>"+curItem.getLastMod()+"</td></tr>";
					jQuery.ajax({
						  type: 'POST',
						  url: '/fileManager/isLink?time='+ new Date().getTime(),
						  data: 'filename='+curItem.path+'&id='+curItem.id,
						  dataType: 'json',
		...
		doGSAction: function(o) {
			...
			if (o.action == '14') { // show upload
				...
				$('#btnUploadClose').button('option', 'class', 'test');
				var buttons = $('#gsuploadfiles').parent().find('.ui-dialog-buttonset').children('button');
				$(buttons[0]).attr('class','').addClass('btn btn-inverse');
				$(buttons[1]).attr('class','').addClass('btn btn-inverse');
				return;
			}
			...
			if(o.action == '46'){ //Share
				shareItem(o, curDir, gsitem);
				return;
			}
			...
			function showCKEditor(o, curDir, gsitem){
				...
				var buttons = $('#gsckeditor').parent().find('.ui-dialog-buttonset').children('button');
				$(buttons[0]).attr('class','').addClass('btn btn-inverse');
				$(buttons[1]).attr('class','').addClass('btn btn-inverse');
			...
			function showNotePad(o, curDir, gsitem){
				...
				var buttons = $('#gsnotepadedit').parent().find('.ui-dialog-buttonset').children('button');
				$(buttons[0]).attr('class','').addClass('btn btn-inverse');
				$(buttons[1]).attr('class','').addClass('btn btn-inverse');
			...
			function showImageViewer(o, curDir, gsitem){
				...
				var buttons = $('#gsimageviewer').parent().find('.ui-dialog-buttonset').children('button');
				$(buttons[0]).attr('class','').addClass('btn btn-inverse');
				$(buttons[1]).attr('class','').addClass('btn btn-inverse');
			...
			function showJcrop(o, curDir, gsitem){
				...
				var buttons = $('#gs_jcrop_div').parent().find('.ui-dialog-buttonset').children('button');
				$(buttons[0]).attr('class','').addClass('btn btn-inverse');
				$(buttons[1]).attr('class','').addClass('btn btn-inverse');
				jQuery('#gs_jcrop_div_container').html('<div class="loadingDiv">&nbsp;</div>');
				dataForSend = {opt: 15, filename: gsitem.name, dir: curDir};
			...
			function shareItem(o, curDir, gsitem){ // Share Item
				//ShareFormFields
				jQuery.ajax({
	      		      type: 'POST',
					  url: '/fileManager/getUserList',					 
					  success: function(data) {
					             $('#ShareFormFields').html(data);
	      	   					}
			   });

				jQuery('#sharefiles').dialog({title: gs_getTranslation(o.lg, 46), modal: true, width: 460, height: 460,
					buttons: [ {text: gs_getTranslation(o.lg, 28), 
								"class": "btn btn-inverse",
								"id" : 'btnUploadClose',
						        click: function() { 
					           ...
					           {
						       text: gs_getTranslation(o.lg, 45),
								"id" : 'btnShareSave',
								"class": "btn btn-inverse",
								click: function() {								  
				    	         // jQuery('#gsShareForm').submit(); 
				                }
					         }]
				});
				
				jQuery('#btnShareSave').click(function (e){
				e.stopPropagation();
				$('#ShareError').html('');

					var selectedFiles = gsGetSelectedItems();
					var ShareIds = $('#ShareId').val();

					if (ShareIds) {
	   					var ids= ShareIds.join(',');
					}else{
						$('#ShareError').html(gs_getTranslation(o.lg, 47));	
						return;
					}

	            	  jQuery(this).find("input[name=dir]").val(curDir);
					  jQuery(this).find("input[name=filename]").val(selectedFiles);
					
					jQuery.ajax({
	      		      type: 'POST',
					  url: '/fileManager/shareFile',
					  data: 'dir='+curDir+'&filename='+selectedFiles+'&ShareIds='+ids+'&curDir='+curDir,
					  dataType: 'json',
					  contentType : 'application/x-www-form-urlencoded; charset=utf-8',
					  success: function(data) {
						  	jQuery('#'+jQuery("#curDir").attr('rel')).trigger('click');
					...
					jQuery('#sharefiles').dialog("close");
					
					});
				
				$('#btnShareSave').button('option', 'class', 'test');
				var buttons = $('#sharefiles').parent().find('.ui-dialog-buttonset').children('button');
				$(buttons[0]).attr('class','').addClass('btn btn-inverse');
				$(buttons[1]).attr('class','').addClass('btn btn-inverse');
				return;
			...
			function deleteItem(o, curDir, gsitem){
				...
				jQuery.ajax({
					...
					data: 'dir='+curDir+'&filename='+selectedFiles,
					dataType: 'json',
					contentType : 'application/x-www-form-urlencoded; charset=utf-8',
					success: function(data) {						
						for(var i=0; i<data.length; i++)
						{
							...
							if(dataArray.error)
							{
								selectedFiles = selectedFiles.replace(dataArray.filename,"");								
								alert(dataArray.msg+" : "+dataArray.filename);
							}
							else
							{
								if (dataArray.filename != null) {
									dataForSend = {opt: 4, files: encodeURIComponent(dataArray.filename), dir: curDir};
								}
								sendAndRefresh(o, dataForSend, true);
							}
						}						
...
if(jQuery)( function() {
	...
	$(window).bind('load', function(){
		//$('.ui-dialog-buttonpane. ui-dialog-buttonset .ui-button').attr('class','btn btn-inverse');
		$('.gs_dir_content_menu a').addClass('btn btn-inverse');
		$('#gs_invertselectbutton').hide();
		$('#btnUploadClose').attr('class','').addClass('btn btn-inverse');
	});
